...
"""Selected India climates- all relative humidity data is for 17:30 IST

https://en.wikipedia.org/wiki/Mount_Abu#Climate

- highs_mth = np.array([19.7,22.3,26.5,30.6,32.3,30.2,24.9,23.5,25.6,27.5,24.2,21.5])
- lows_mth = np.array([3.1,5.7,10.5,15.3,18.5,18.3,17.3,16.6,15.8,11.9,7.1,3.8])
- avgs_mth = np.array([11.4,14,18.5,22.9,25.4,24.3,21.1,20,20.7,19.7,15.7,12.6])
- #dewpoint_mth = np.array([31.7,31.8,30.2,24.5,17.3,12.4,11.2,13.7,17.8,26,30.8,32.1])
- rel_humid_mth = np.array([47,38,34,32,38,55,84,90,76,53,49,49])

- precip_mth = np.array([3.5,1.6,2,3,10.4,61.9,592.9,516.3,172.4,17.7,3.4,1])
- precip_days_mth = np.array([0.3,0.1,0.3,0.3,0.8,3.7,14.6,15.7,6.1,0.7,0.3,0.1])

- rec_high_mth = np.array([29,30.6,39.6,38.8,40.4,38.4,35,31.1,33,33.6,30.4,34.2])
- rec_low_mth = np.array([-7,-5.8,0.4,4.4,10,13,10,10.6,6.4,3.4,-0.4,-7.4])

climate_name = "MountAbu_2010"


https://en.wikipedia.org/wiki/Surat#Climate

- highs_mth = np.array([30.2,32.2,35.4,36.6,35.7,33.8,31.1,30.8,32,34.9,34.1,31.6])
- lows_mth = np.array([15.2,17.2,20.9,24.4,27.4,27.3,26.2,25.9,25.6,23.5,19.9,16.6])
- avgs_mth = np.array([22.6,24.7,28,30.3,31.4,30.3,28.7,28.1,28.6,29,26.8,23.7])
- (# Surat's dew point data might actually be from Ahmedabad?
- #dewpoint_mth = np.array([9,10,10,14,19,23,25,25,24,19,14,11])
- rel_humid_mth = np.array([41,35,32,43,61,72,81,80,72,54,46,45])
- precip_mth = np.array([1.9,0.3,0.7,0.5,2.4,255.9,466.3,281.7,186.7,40.7,5.1,1.1])
- precip_days_mth = np.array([0.2,0.1,0.1,0.1,0.2,8,15,12.3,8.1,2,0.5,0.1])

- rec_high_mth = np.array([38.3,41.7,44,45.6,45.6,45.6,38.9,37.2,41.1,41.4,39.4,38.9])
- rec_low_mth = np.array([4.4,5.6,8.9,15,19.4,20.2,19.9,20.2,20.6,14.4,10.6,6.7])

climate_name = "Surat_2020"




https://en.wikipedia.org/wiki/Chennai#Environment

- highs_mth = np.array([29.3,30.9,32.9,34.5,37.1,37,35.3,34.7,34.2,32.1,29.9,28.9])
- lows_mth = np.array([21.2,22.2,24.2,26.6,28,27.5,26.4,25.9,25.6,24.6,23.1,21.9])
- avgs_mth = np.array([25.4,26.7,28.7,31,33,32.3,31,30.3,29.8,28.5,26.7,25.6])
- #dewpoint_mth = np.array([9,10,10,14,19,23,25,25,24,19,14,11])
- rel_humid_mth = np.array([67,66,67,70,68,63,65,66,71,76,76,71])

- precip_mth = np.array([20,4.7,3.4,17.5,49.7,75.4,113.1,141.4,143.9,278.3,377.3,183.7])
- precip_days_mth = np.array([1.4,0.6,0.2,1,1.8,4.5,6.7,8.8,7.4,10.6,11.5,5.7])

- rec_high_mth = np.array([34.4,36.7,40.6,42.8,45,43.3,41.1,40,38.9,39.4,35.4,33])
- rec_low_mth = np.array([13.9,15,16.7,20,21.1,20.6,21,20.5,20.6,16.7,15,13.9])

climate_name = "Chennai_2020"

https://weatherspark.com/h/d/110123/2023/5/17/Historical-Weather-on-Wednesday-May-17-2023-in-Chennai-India#Figures-Humidity dew point reading of 30 C (86 F)


https://en.wikipedia.org/wiki/Kochi#Climate

- highs_mth = np.array([31.9,32,32.6,33,32.4,30.3,29.6,29.5,30.2,30.7,31.3,31.9])
- lows_mth = np.array([23,24.2,25.5,25.9,25.7,24.2,23.8,24,24.2,24.1,24.1,23.2])

- #dewpoint_mth = np.array([9,10,10,14,19,23,25,25,24,19,14,11])
- rel_humid_mth = np.array([61,65,68,70,73,82,83,82,79,77,72,64])

- precip_mth = np.array([24.3,27.1,45,113.1,284.5,700.3,575.5,378.8,310.3,366.6,150.4,39])
- precip_days_mth = np.array([1.1,1.2,2.6,6.9,11,23,22.8,19,13.4,14.2,7.2,1.8])

- rec_high_mth = np.array([36.4,35.7,36,36.5,35.2,34.2,33.1,32.5,34.2,34.6,35,35.2])
- rec_low_mth = np.array([16.5,16.3,21.6,21.2,21.1,20.4,17.6,20.6,21.1,19.2,19.2,17.7])

climate_name = "Kochi2020"




https://en.wikipedia.org/wiki/Ariyalur#Climate

Cool that this table has avg max/min per month... also the precip days is for days with >2.5 mm only...

- highs_mth = np.array([31.7,33.6,36.2,38,38.7,37,35.9,36.4,36.2,34.2,32,30.8])
- lows_mth = np.array([21.4,22.4,24.3,28,27.7,27.9,27.5,26.8,25.8,24.3,22.7,21.7])
- avgs_mth = np.array([26.6,28,30.3,33,33.2,32.5,31.7,31.6,31,29.3,27.4,26.3])

- #dewpoint_mth = np.array([9,10,10,14,19,23,25,25,24,19,14,11])
- rel_humid_mth = np.array([80,72,63,61,61,61,62,66,71,80,85,83])

- precip_mth = np.array([2.6,0.9,8.6,18.5,51.3,32.4,22.8,88.7,83.5,114.7,190.9,93.2])
- precip_days_mth = np.array([0.4,0.1,0.2,1,2.5,1.9,1.4,4.2,4.8,6.7,7.7,4.2])

- rec_high_mth = np.array([42,42.4,42.4,45.4,48.4,42,41.5,49.6,44.4,42.2,42.2,42.4])
- rec_low_mth = np.array([15.2,13,15.2,20,15.6,21.9,19.4,20.6,19.8,18.2,13,15.2])

- high_max_mth = np.array([33.9,35.8,39,40.6,40.8,39.5,38.9,39.3,38.9,37,35.3,33.8])
- low_min_mth = np.array([19.9,20.8,21,24.6,23.5,25.3,25.2,23.9,23.3,22.2,20.8,20.2])

climate_name = "Ariyalur2020"

#Non-Subcontinent climates

- highs_mth = np.array([31.6,33.2,34.6,35.3,34.8,33.8,32.9,32.7,32.2,31.4,31.1,30.8])
- lows_mth = np.array([21.8,22.8,24.3,25.5,25.6,24.9,24.8,24.6,24.4,24.2,23.2,21.9])
- avgs_mth = np.array([26.6,28,29.4,30.2,30,29.2,28.7,28.5,28.2,27.2,27.1,26.3])
- #dewpoint_mth = np.array([9,10,10,14,19,23,25,25,24,19,14,11])
- rel_humid_mth = np.array([73,71,71,73,77,78,80,81,84,84,78,73])

- precip_mth = np.array([12.1,6.6,34.8,78.8,118.2,145,162.1,182.7,270.9,248.1,120.5,32.1])
- precip_days_mth = np.array([1.2,1.1,3.4,6.8,15.9,17,18.1,18.3,21.5,19.3,10.2,4.5])

- rec_high_mth = np.array([36.1,38.1,40,40.5,40,39.2,37.2,37.8,35.5,36.1,34.4,37.2])
- rec_low_mth = np.array([12.8,15.2,19,17.8,20.6,21.2,20.1,20,21.1,17.2,16.7,14.4])

climate_name = "PhnomPenh2013_25"


https://en.wikipedia.org/wiki/Teresina#Climate

- highs_mth = np.array([32.5,32,31.9,31.8,32.2,32.5,33.6,35.4,36.6,37.2,36.2,34.8])
- lows_mth = np.array([22.8,22.6,22.8,22.8,22.6,21.5,20.8,21.1,22.1,23.2,23.6,23.4])
- dewpoint_mth = np.array([23.5,23.8,24,24.3,23.8,22.3,21,20.4,20,20.7,21.5,22.5])
- rel_humid_mth = np.array([80.2,83.6,85.1,85.4,83.4,76.9,69.4,61.8,56.6,56.2,61.8,69.8])

- precip_mth = np.array([196.8,239.1,286.9,265.7,120.6,14.7,10.6,12.1,13,19.5,45.9,100.1])
- precip_days_mth = np.array([11.8,13.7,18.8,16.6,9.6,2.3,1.6,1.4,1.8,2.2,3.8,7.2])

- rec_high_mth = np.array([38.4,38,39.6,35,35.8,36.9,37.6,38.4,39.6,40.4,40.8,39.3])
- rec_low_mth = np.array([19.8,20,18.5,19,19.4,16.6,15.7,16.3,17.7,18,19,18.4])

climate_name = "TeresinaBR_2010"


https://en.wikipedia.org/wiki/Toksun_County#Geography_and_Climate

- highs_mth = np.array([-2.0,7.3,18.1,27.5,33.5,38.6,40,38.2,32.1,22.6,10.6,0])
- lows_mth = np.array([-12.4,-5.6,3.9,13.2,19,24.4,25.7,23.4,16.7,7.7,-1.3,-9.6])
- avgs_mth = np.array([-7.9,0.3,10.7,20.1,26.1,31.4,32.6,30.4,23.7,14,3.7,-5.5])
- #dewpoint_mth = np.array([23.5,23.8,24,24.3,23.8,22.3,21,20.4,20,20.7,21.5,22.5])
- rel_humid_mth = np.array([60,43,29,26,26,29,33,36,39,47,52,61])

- precip_mth = np.array([0.5,0,0.5,0.4,0.4,2.1,2,1.5,0.7,0.8,0,0.3])
- precip_days_mth = np.array([0.7,0.1,0.1,0.3,0.5,1.5,1.9,1.7,0.7,0.3,0.1,0.5])

- rec_high_mth = np.array([12.4,18.6,32.2,42,44.3,48,47.6,47.4,41.3,35.1,21.9,13.1])
- rec_low_mth = np.array([-19.8,-17.1,-11.6,0,5.4,11.6,14.6,10.8,4.4,-3.5,-18.2,-21.8])

climate_name = "ToksunCN_2010"
"""
...
"""##### https://en.wikipedia.org/wiki/Eyre_Bird_Observatory#Climate

dewpoint_mth = np.zeros(12)
rel_humid_mth = np.zeros(12)

highs_mth = np.array([26.7,26.7,25.8,24.3,21.5,19.1,18.4,19.6,21.5,23.3,24.5,25.8])
lows_mth = np.array([15.3,15.9,14.3,11.7,8.9,6.7,5.6,6,7.6,10.1,12.1,14])
avgs_mth = (highs_mth + lows_mth)/2
#####dewpoint_mth = np.array([23.5,23.8,24,24.3,23.8,22.3,21,20.4,20,20.7,21.5,22.5])
rel_humid_mth = np.array([61,62,62,61,59,57,57,55,56,58,62,62])

precip_mth = np.array([18.1,18.8,26.6,24.6,37.9,39.1,33.9,30.1,23.7,20.5,22.4,20.7])
precip_days_mth = np.array([4.2,4.6,7.1,7.3,10.2,10.3,10.5,9.5,7.6,6.8,6.2,5.1])

rec_high_mth = np.array([48.5,49.1,45.7,41.5,37.5,30.4,31,35.4,38.5,44.7,45.4,47.5])
rec_low_mth = np.array([2.5,1.5,0.5,-2.0,-2.9,-6.2,-5.9,-7.2,-5.5,-5.0,-2.1,0.2])

climate_name = "EyreBirdObservatory_WA-Aus_Wiki"


##### remember to manually adjust the yticks range!
ax.set_yticks(np.arange(-10, 50, 5.0))
